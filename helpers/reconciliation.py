...
class ReconciliationEngine:
    ...
    def verify_totals_and_subtotals(
        self, data: Dict[str, Any], report_type: str = "auto"
        ...
        consistency_issues = self._check_consistency(
            data, rules.get("consistency_checks", [])
        )
        result.issues.extend(consistency_issues)

        # Calculate score
        total_checks = (
            len(rules.get("required_fields", [])) + len(rules.get("subtotals", {})) + 1
        )
    ...
    def _detect_report_type(self, data: Dict[str, Any]) -> str:
        ...
        pnl_score = sum(
            1
            for indicator in pnl_indicators
    ...
    def _verify_subtotals(
        self, data: Dict[str, Any], subtotals: Dict[str, str]
    ) -> List[str]:
        ...
        for subtotal_name, formula in subtotals.items():
            ...
            if missing_components:
                issues.append(
                    f"Missing components for {subtotal_name}: {missing_components}"
                )
                continue
            ...
            try:
                ...
                if abs(actual_value - expected_value) > self.tolerance:
                    issues.append(
                        f"Subtotal mismatch for {subtotal_name}: expected {expected_value}, got {actual_value}"
                    )
            except (ValueError, TypeError):
                issues.append(
                    f"Invalid subtotal value for {subtotal_name}: {data[subtotal_name]}"
                )
    ...
    def _verify_total_formula(self, data: Dict[str, Any], formula: str) -> List[str]:
        ...
        if abs(left_value - right_value) > self.tolerance:
            issues.append(
                f"Total formula mismatch: {left_side} ({left_value}) != {right_side} ({right_value})"
            )
            ...
            if ">=" in rule:
                ...
                if field in data:
                    try:
                        ...
                        if value < threshold:
                            issues.append(
                                f"Consistency violation: {field} ({value}) < {threshold}"
                            )
                    except (ValueError, TypeError):
                        issues.append(
                            f"Invalid value for consistency check: {field} = {data[field]}"
                        )

        return issues

    def _generate_suggestions(
        self, data: Dict[str, Any], rules: Dict[str, Any], issues: List[str]
    ) -> List[str]:
        ...
        for issue in issues:
            if "Missing required fields" in issue:
                ...
                suggestions.append(
                    "Check for data entry errors or missing transactions"
                )
            elif "Consistency violation" in issue:
                suggestions.append(
                    "Review negative values and ensure they are intentional"
                )
            elif "Invalid value" in issue:
                suggestions.append(
                    "Check data types and ensure numerical values are properly formatted"
                )
...
def reconcile_totals(
    data: Dict[str, Any], report_type: str = "auto", tolerance: float = 0.01
) -> Dict[str, Any]:
...
def cross_field_consistency_check(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for normalized, fields in field_groups.items():
        if len(fields) > 1:
            ...
            if len(set(values)) > 1:
                issues.append(
                    f"Conflicting values for similar fields: {[f[0] for f in fields]}"
                )
                suggestions.append(
                    f"Review and standardize field naming for {normalized}"
                )
    ...
    if "revenue" in data and "expenses" in data and "net_profit" in data:
        try:
            ...
            if abs(net_profit - expected_profit) > 0.01:
                issues.append(
                    f"Net profit ({net_profit}) doesn't match revenue ({revenue}) - expenses ({expenses})"
                )
                suggestions.append(
                    "Recalculate net profit or review revenue/expense values"
                )
        except (ValueError, TypeError):
            pass
...
def detect_mismatches(
    ...
    return {
        ...
        "summary": {
            ...
            "numerical_mismatches": len(
                [m for m in mismatches if m["type"] == "numerical"]
            ),
...
def generate_correction_suggestions(mismatches: List[Dict[str, Any]]) -> List[str]:
    ...
    if numerical_mismatches:
        suggestions.append(
            f"Found {len(numerical_mismatches)} numerical mismatches - review calculations"
        )

    if text_mismatches:
        suggestions.append(
            f"Found {len(text_mismatches)} text mismatches - check field mappings"
        )

    if missing_fields:
        suggestions.append(
            f"Found {len(missing_fields)} missing fields - ensure complete data extraction"
        )
    ...
    for mismatch in mismatches[:5]:  # Limit to first 5 for readability
        ...
        if "revenue" in field.lower() or "income" in field.lower():
            ...
            suggestions.append(
                f"Verify {field} calculation against revenue and expenses"
            )
