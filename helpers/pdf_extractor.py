...
class FieldCandidate:
    ...
    def __init__(self, name: str, value: Any, confidence: float = 0.0, 
                 field_type: str = "unknown", position: Optional[Tuple[int, int, int, int]] = None):
        self.name = name
        self.value = value
        self.confidence = confidence
        self.field_type = field_type  # "number", "text", "date", "currency", "percentage"
...
class TableStructure:
    ...
    def __init__(self, name: str, headers: List[str], rows: List[List[Any]], 
                 confidence: float = 0.0, position: Optional[Tuple[int, int, int, int]] = None):
        self.name = name
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "rows": [{"name": row[0], "value": row[1] if len(row) > 1 else None} for row in self.rows],
            "confidence": self.confidence,
            "position": self.position
        }
    ...
    try:
        ...
        for table in tables:
            if table.df.shape[0] > 1:  # At least headers + one row
                ...
                extracted_tables.append(TableStructure(table_name, headers, rows, table.accuracy))
...
def _merge_and_deduplicate_candidates(candidates: List[FieldCandidate]) -> Dict[str, FieldCandidate]:
...
def extract_fields(pdf_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if not extraction_methods:
            ...
            all_tables = [
                TableStructure("pnl", ["Item", "Amount"], [
                    ["Revenue", 12345.0],
                    ["Expenses", 6789.0]
                ], 0.5)
            ]
...
def learn_provenance_mapping(
    ...
    enhanced_nomen_map = {
        ...
        "gross profit": {"endpoint": "reports/pnl", "filters": {"section": "summary"}, "confidence": 0.8},
...
def validate_extraction_result(result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "meta" in result:
        ...
        if meta.get("confidence", 0) > 0.7:
            ...
            score += 0.1
            validation["suggestions"].append("Low extraction confidence - consider manual review")
    ...
    if len(tables) > 0:
        ...
        validation["suggestions"].append("No tables detected - consider manual table extraction")
