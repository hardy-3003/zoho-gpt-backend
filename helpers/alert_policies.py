...
class AlertManager:
    ...
    def __init__(self):
        ...
        self.escalation_cooldown = int(os.getenv("ALERT_ESCALATION_COOLDOWN_SEC", "1800"))
    ...
    def _evaluate_policy(self, policy: AlertPolicy, snapshot, current_time: float) -> List[AlertEvent]:
        ...
        for threshold in policy.thresholds:
            if self._is_threshold_breached(policy, threshold, current_value, current_time):
                event = self._create_alert_event(policy, threshold, current_value, current_time)
                events.append(event)
    ...
    def _is_threshold_breached(self, policy: AlertPolicy, threshold: Threshold, 
                              current_value: float, current_time: float) -> bool:
        ...
        if state:
            duration_exceeded = (current_time - state.first_triggered) >= threshold.duration_sec
    ...
    def _create_alert_event(self, policy: AlertPolicy, threshold: Threshold,
                           current_value: float, current_time: float) -> AlertEvent:
        ...
        message = (f"Metric '{policy.match.get('metric', 'unknown')}' is "
...
def evaluate_slo_alerts(slo_results: List[SLOResult]) -> List[AlertEvent]:
    ...
    for result in slo_results:
        ...
        for alert in burn_alerts:
            event = AlertEvent(
                id=hashlib.md5(f"slo_{result.spec.id}_{alert['severity']}".encode()).hexdigest(),
                ...
                metadata={"slo_id": result.spec.id, "slo_type": result.spec.slo_type.value}
            )
            events.append(event)
