...
from helpers.schema_registry import save_learned_format, validate_learned_format, load_learned_format, list_learned_formats
from helpers.provenance import get_global_learner, learn_from_pdf_extraction, make_field_provenance
from helpers.history_store import log_with_deltas_and_anomalies
from helpers.learning_hooks import score_confidence
from helpers.reconciliation import reconcile_totals, detect_mismatches, cross_field_consistency_check
...
ENHANCED_NOMEN_MAP: Dict[str, Dict[str, Any]] = {
    "Revenue": {"endpoint": "reports/pnl", "filters": {"section": "income"}, "confidence": 0.9},
    "Income": {"endpoint": "reports/pnl", "filters": {"section": "income"}, "confidence": 0.9},
    "Sales": {"endpoint": "reports/pnl", "filters": {"section": "income"}, "confidence": 0.8},
    "Turnover": {"endpoint": "reports/pnl", "filters": {"section": "income"}, "confidence": 0.8},
    "Expenses": {"endpoint": "reports/pnl", "filters": {"section": "expense"}, "confidence": 0.9},
    "Costs": {"endpoint": "reports/pnl", "filters": {"section": "expense"}, "confidence": 0.8},
    "Expenditure": {"endpoint": "reports/pnl", "filters": {"section": "expense"}, "confidence": 0.8},
    "Net Profit": {"endpoint": "reports/pnl", "filters": {"section": "summary"}, "confidence": 0.9},
    "Profit": {"endpoint": "reports/pnl", "filters": {"section": "summary"}, "confidence": 0.9},
    "Loss": {"endpoint": "reports/pnl", "filters": {"section": "summary"}, "confidence": 0.9},
    "Gross Profit": {"endpoint": "reports/pnl", "filters": {"section": "summary", "type": "gross"}, "confidence": 0.8},
    "Operating Profit": {"endpoint": "reports/pnl", "filters": {"section": "summary", "type": "operating"}, "confidence": 0.8},
    "Assets": {"endpoint": "reports/balance_sheet", "filters": {"section": "assets"}, "confidence": 0.9},
    "Liabilities": {"endpoint": "reports/balance_sheet", "filters": {"section": "liabilities"}, "confidence": 0.9},
    "Equity": {"endpoint": "reports/balance_sheet", "filters": {"section": "equity"}, "confidence": 0.9},
    "Capital": {"endpoint": "reports/balance_sheet", "filters": {"section": "equity"}, "confidence": 0.8},
    ...
    try:
        ...
        return {
            ...
            "summary": {
                ...
                "extraction_confidence": extraction_result.get("meta", {}).get("confidence", 0.0),
                "format_confidence": format_validation.get("confidence", 0.0),
                "overall_quality": (validation.get("score", 0.0) + format_validation.get("score", 0.0)) / 2
            }
        }
        ...
        return {
            "error": str(e),
            "meta": {"file": pdf_path, "pages": 1, "extraction_methods": ["error"], "confidence": 0.0},
            "mapping_path": "",
            "mapping": {},
            "extraction_validation": {"is_valid": False, "score": 0.0, "issues": [str(e)]},
...
def generate_from_learned(
    payload: Dict[str, Any], format_name: str, 
    validate_output: bool = True, auto_correct: bool = False
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if not learned_format:
            return {
                ...
                "provenance": {},
                "confidence": 0.0,
                "alerts": [{"level": "error", "msg": f"Format {format_name} not found"}]
            }
        ...
        if validate_output:
            ...
            if not format_validation.get("is_valid", True):
                alerts.append({"level": "warning", "msg": f"Format validation failed: {format_validation.get('errors', [])}"})
            
            if not reconciliation.get("reconciliation", {}).get("is_valid", True):
                alerts.append({"level": "warning", "msg": f"Reconciliation issues: {reconciliation.get('reconciliation', {}).get('issues', [])}"})
            
            if not consistency.get("is_consistent", True):
                alerts.append({"level": "warning", "msg": f"Consistency issues: {consistency.get('issues', [])}"})
        ...
        if auto_correct and validation_results:
            ...
            if corrections:
                ...
                alerts.append({"level": "info", "msg": f"Applied {len(corrections)} auto-corrections"})
        ...
        if validation_results.get("format_validation"):
            confidence_factors.append(validation_results["format_validation"].get("score", 0.0))
        
        # Reconciliation confidence
        if validation_results.get("reconciliation"):
            confidence_factors.append(validation_results["reconciliation"]["reconciliation"].get("score", 0.0))
        
        # Consistency confidence
        if validation_results.get("consistency"):
            consistency_score = 1.0 if validation_results["consistency"].get("is_consistent", True) else 0.5
            confidence_factors.append(consistency_score)
        
        # Average confidence
        overall_confidence = sum(confidence_factors) / len(confidence_factors) if confidence_factors else 0.5
        
        # Step 7: Log history and deltas
        ...
        return {
            ...
            "meta": {
                ...
                "format_version": learned_format.get("version", "1.0"),
...
def _apply_auto_corrections(data: Dict[str, Any], validation_results: Dict[str, Any]) -> Dict[str, Any]:
...
def list_available_formats() -> Dict[str, Any]:
    ...
    try:
        ...
        avg_confidence = sum(fmt.get("confidence", 0.0) for fmt in formats) / total_formats if total_formats > 0 else 0.0
        ...
        return {
            ...
            "statistics": {
                ...
                "by_source": {source: len(formats) for source, formats in by_source.items()}
            },
