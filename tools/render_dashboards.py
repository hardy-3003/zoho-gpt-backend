...
def validate_panel(panel: Dict[str, Any], index: int) -> List[str]:
    ...
    # Validate panel type
    valid_types = ["stat", "timeseries", "table", "piechart", "gauge", "heatmap", "graph"]
    if "type" in panel:
...
def validate_target(target: Dict[str, Any], panel_index: int, target_index: int) -> List[str]:
    ...
    for field in required_fields:
        if field not in target:
            errors.append(f"Panel {panel_index}, Target {target_index}: Missing required field: {field}")
    
    # Validate query format
    if "query" in target:
        query = target["query"]
        if not isinstance(query, str) or not query.strip():
            errors.append(f"Panel {panel_index}, Target {target_index}: Invalid query: {query}")
    ...
    if "format" in target:
        ...
        if format_val not in valid_formats:
            errors.append(f"Panel {panel_index}, Target {target_index}: Invalid format: {format_val}")
...
def validate_position(position: Dict[str, Any], panel_index: int) -> List[str]:
    ...
    for field in required_fields:
        if field not in position:
            ...
            errors.append(f"Panel {panel_index}: Position field {field} must be integer")
...
def analyze_dashboard(dashboard_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for panel in panels:
        ...
        analysis["panel_types"][panel_type] = analysis["panel_types"].get(panel_type, 0) + 1
...
Examples:
  ...
  python tools/render_dashboards.py --summary dashboards/*.json
    ...
    parser.add_argument(
        "--validate",
        action="store_true",
        help="Validate dashboard schemas"
    )
    ...
    if args.summary and all_analyses:
        ...
        for filepath, analysis in all_analyses:
            ...
            print(f"     Panels: {analysis['total_panels']}, Queries: {len(analysis['queries'])}")
