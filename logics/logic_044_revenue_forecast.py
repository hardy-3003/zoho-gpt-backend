...
ID: L-044
Tags: []
Required Inputs: schema://revenue_forecast.input.v1
Outputs: schema://revenue_forecast.output.v1
Assumptions:
...
LOGIC_META = {
    "id": "L-044",
    "title": "Revenue Forecast",
    "tags": ["forecast", "revenue", "projection"],
}


def _validate_revenue_forecast(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    forecast = result.get("forecast", [])
    if any((f.get("revenue", 0.0) or 0.0) < 0 for f in forecast):
        alerts.append("negative_revenue")
    if len(forecast) < 3:
        alerts.append("data_scarce")
    return alerts
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        estimates_url = f"{api_domain}/books/v3/estimates?date_start={start_date}&date_end={end_date}&organization_id={org_id}"
        sources.extend([invoices_url, estimates_url])
        _ = get_json(invoices_url, headers)
        _ = get_json(estimates_url, headers)

        result = {
            "period": {"start": start_date, "end": end_date},
            "forecast": [],
            "method": "simple-moving-avg",
        }
    ...
    alerts = _validate_revenue_forecast(result)
