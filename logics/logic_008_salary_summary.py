...
ID: L-008
Tags: []
Required Inputs: schema://salary_summary.input.v1
Outputs: schema://salary_summary.output.v1
Assumptions: 
...
LOGIC_META = {
    "id": "L-008",
    "title": "Salary Summary",
    "tags": ["salary", "payroll", "hr", "employees"],
}


def _validate_salary_summary(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    try:
        totals = result.get("totals", {}) or {}
        amount = float(totals.get("amount", 0) or 0)
        if amount < 0:
            alerts.append("negative total amount")
        for emp in result.get("employees", []) or []:
            if float(emp.get("amount", 0) or 0) < 0:
                alerts.append("negative employee amount")
                break
    except Exception:
...
def handle(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        sources.append("/books/v3/bills?search=salary")
        result = {
    ...
    alerts = _validate_salary_summary(result)
    learn = _learn_from_history(payload, result)
    conf = 0.6
    if any(a.startswith("negative") for a in alerts):
        conf -= 0.15
    ...
    try:
        ...
        pack = log_with_deltas_and_anomalies(
            "L-008", payload, out.get("result") or {}, prov
        )
        if pack.get("alerts"):
            out["alerts"] = list(out.get("alerts", [])) + pack.get("alerts", [])
        new_conf = _score(
            sample_size=max(
                1, len((out.get("result") or {}).get("employees", []) or [])
            ),
