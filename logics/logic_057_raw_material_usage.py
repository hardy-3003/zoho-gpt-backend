...
ID: L-057
Tags: []
Required Inputs: schema://raw_material_usage.input.v1
Outputs: schema://raw_material_usage.output.v1
Assumptions:
...
LOGIC_META = {
    "id": "L-057",
    "title": "Raw Material Usage",
    "tags": ["inventory", "rm", "usage", "production"],
}


def _validate_rm_usage(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    mats = result.get("materials", [])
    total = float(result.get("totals", {}).get("qty_used", 0.0) or 0.0)
    sum_used = 0.0
    for m in mats:
        used = float(m.get("qty_used", 0.0) or 0.0)
        std = m.get("std_qty")
        var = m.get("variance")
        if used < 0:
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        usage_url = f"{api_domain}/books/v3/inventory/consumption?date_start={start_date}&date_end={end_date}&organization_id={org_id}"
        sources.append(usage_url)
        _ = get_json(usage_url, headers)
    ...
    alerts = _validate_rm_usage(result)
