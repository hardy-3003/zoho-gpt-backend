...
LOGIC_META = {
    "id": "L-200",
    "title": "Vendor-side Carbon Emissions Estimator",
    "tags": ["esg", "carbon", "vendor"],
}


def _validate_co2e(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    for row in result.get("vendors", []) or []:
        try:
            spend = float(row.get("spend", 0.0))
            factor = float(row.get("emission_factor", 0.0))
            co2e = float(row.get("co2e", 0.0))
            if spend < 0 or factor < 0 or co2e < 0:
                alerts.append("negative_values")
            if abs(co2e - spend * factor) > 0.0001:
                alerts.append("co2e_mismatch")
        except Exception:
            alerts.append("invalid_vendor_row")
