...
LOGIC_META = {
    "id": "L-073",
    "title": "Compliance Tracker",
    "tags": ["compliance", "filings", "due"],
}

_STATUSES = {"filed", "pending", "overdue"}


def _validate_compliance(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    items = result.get("items", [])
    totals = result.get("totals", {})
    overdue_count = sum(1 for it in items if it.get("status") == "overdue")
    if int(totals.get("overdue", 0) or 0) != overdue_count:
        alerts.append("overdue_totals_mismatch")
    for it in items:
        if it.get("status") not in _STATUSES:
            alerts.append("invalid_status")
    if overdue_count > 0:
        alerts.append("has_overdue")
    return alerts
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        filings_url = f"{api_domain}/books/v3/taxes/returns?date_start={start_date}&date_end={end_date}&organization_id={org_id}"
        sources.append(filings_url)
        _ = get_json(filings_url, headers)
    ...
    alerts = _validate_compliance(result)
