...
LOGIC_META = {
    "id": "L-069",
    "title": "Business Vertical Summary",
    "tags": ["vertical", "segment", "summary"],
}


def _validate_vertical_summary(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    segs = result.get("verticals", [])
    totals = result.get("totals", {})
    sum_inc = sum(float((d.get("income", 0.0) or 0.0)) for d in segs)
    sum_exp = sum(float((d.get("expense", 0.0) or 0.0)) for d in segs)
    sum_profit = sum(float((d.get("profit", 0.0) or 0.0)) for d in segs)
    ...
    for d in segs:
        ...
        if abs(pr - (inc - exp)) > 0.01:
            alerts.append(f"profit_mismatch:{d.get('vertical','')}")
    return alerts
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    alerts = _validate_vertical_summary(result)
