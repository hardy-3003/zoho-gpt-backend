...
LOGIC_META = {
    "id": "L-192",
    "title": "Suggested Journal Entry Fixes",
    "tags": ["journal", "fixes", "suggestion"],
}


def _validate_je(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    for row in result.get("suggestions", []) or []:
        if not all(k in row for k in ["je_id", "issue", "suggestion"]):
            alerts.append("missing_fields")
