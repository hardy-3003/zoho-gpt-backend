...
"""
Title: Legal Case Cost Monitor
ID: L-171
Tags: []
Required Inputs: schema://legal_case_cost_monitor.input.v1
Outputs: schema://legal_case_cost_monitor.output.v1
Assumptions: 
...
LOGIC_META = {
    "id": "L-171",
    "title": "Legal Case Cost Monitor",
    "tags": ["legal", "case", "provision"],
}


def _validate_lccm(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    cases = result.get("cases") or []
    total_spend = total_prov = 0.0
    for c in cases:
        spend = float(c.get("spend", 0.0) or 0.0)
        prov = c.get("provision")
        if spend < 0:
            alerts.append("negative_spend")
        if prov is not None and float(prov) < 0:
            alerts.append("negative_provision")
        total_spend += spend
        if prov is not None:
            total_prov += float(prov)
    totals = result.get("totals") or {}
    if round(total_spend, 2) != round(float(totals.get("spend", 0.0) or 0.0), 2):
        alerts.append("totals_spend_mismatch")
    if totals.get("provision") is not None and round(total_prov, 2) != round(
        float(totals.get("provision") or 0.0), 2
    ):
        alerts.append("totals_provision_mismatch")
...
def _learn_from_history(
    ...
    try:
        open_cases = sum(
            1 for c in (result.get("cases") or []) if c.get("status") == "open"
        )
        signals.append(f"open_cases:{open_cases}")
        # provision gap if totals.provision < totals.spend * 0.5 (arbitrary example)
        totals = result.get("totals") or {}
        prov = (
            float(totals.get("provision") or 0.0)
            if totals.get("provision") is not None
            else 0.0
        )
        spend = float(totals.get("spend") or 0.0)
        gap = "yes" if prov < (0.5 * spend) and spend > 0 else "no"
        signals.append(f"provision_gap:{gap}")
        if alerts:
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        totals = {
            "spend": round(sum(float(c.get("spend") or 0.0) for c in cases), 2),
            "provision": (
                round(
                    sum(
                        float(c.get("provision") or 0.0)
                        for c in cases
                        if c.get("provision") is not None
                    ),
                    2,
                )
                if cases
        ...
        result = {"as_of": end_date, "cases": cases, "totals": totals}
