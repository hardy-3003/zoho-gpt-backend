...
LOGIC_META = {
    "id": "L-095",
    "title": "Scenario Simulation Engine",
    "tags": ["scenario", "what-if", "planning"],
}


def _validate_scenarios(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    for s in result.get("scenarios", []):
        if not isinstance(s.get("assumptions", {}), dict):
            alerts.append("assumptions_not_dict")
        if not isinstance(s.get("outcome", {}), dict):
            alerts.append("outcome_not_dict")
    return alerts
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    alerts = _validate_scenarios(result)
