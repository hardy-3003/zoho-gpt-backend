...
"""
Title: Creditor Ageing Buckets
ID: L-013
Tags: []
Required Inputs: schema://creditor_ageing_buckets.input.v1
Outputs: schema://creditor_ageing_buckets.output.v1
Assumptions: 
...
LOGIC_META = {
    "id": "L-013",
    "title": "Creditor Ageing Buckets",
    "tags": ["payables", "creditors", "ageing", "ap"],
}


def _validate_creditor_aging(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    try:
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        sources.append("/books/v3/bills?status=unpaid&date_end=")
        result = {
            "as_of": end_date,
            "aging": {
                "0-30": 0.0,
    ...
    alerts = _validate_creditor_aging(result)
    ...
    try:
        ...
        prov["figures"].update(
            make_provenance(
                totals={"endpoint": "reports/payables", "filters": {"as_of": end_date}}
            )
        )
        out["provenance"] = prov

        pack = log_with_deltas_and_anomalies(
            "L-013", payload, out.get("result") or {}, prov
        )
        if pack.get("alerts"):
