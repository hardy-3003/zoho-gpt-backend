...
def _validate_upcoming_bills(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    bills = result.get("bills", [])
    totals = result.get("totals", {})
    if int(totals.get("count", 0) or 0) != len(bills):
        alerts.append("count_mismatch")
    sum_amount = sum(float((x.get("amount", 0.0) or 0.0)) for x in bills)
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        bills_url = f"{api_domain}/books/v3/bills?status=unpaid&due_start={start_date}&due_end={end_date}&organization_id={org_id}"
    ...
    alerts = _validate_upcoming_bills(result)
