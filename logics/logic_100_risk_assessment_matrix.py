...
LOGIC_META = {
    "id": "L-100",
    "title": "Risk Assessment Matrix",
    "tags": ["risk", "matrix", "governance"],
}


def _validate_risk_matrix(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    for r in result.get("risks", []):
        l = int(r.get("likelihood", 0) or 0)
        i = int(r.get("impact", 0) or 0)
        s = int(r.get("score", 0) or 0)
        if not (1 <= l <= 5) or not (1 <= i <= 5):
            alerts.append("likelihood_or_impact_out_of_bounds")
        if s != l * i:
            alerts.append("score_mismatch")
    return alerts
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    alerts = _validate_risk_matrix(result)
