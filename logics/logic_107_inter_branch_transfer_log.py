...
LOGIC_META = {
    "id": "L-107",
    "title": "Inter-Branch Transfer Log",
    "tags": ["ib", "transfer", "inventory", "branches"],
}


def _validate_ib_transfer(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    for t in result.get("transfers", []) or []:
        if (t.get("from") or "") == (t.get("to") or ""):
            alerts.append("from_equals_to")
        try:
            if float(t.get("qty", 0.0)) < 0:
                alerts.append("negative_qty")
        except Exception:
            alerts.append("qty_invalid")
        # simple hint if cost center fields appear but are empty
        if "cost_center" in t and not t.get("cost_center"):
            alerts.append("missing_cost_center")
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        adj_url = f"{api_domain}/books/v3/inventoryadjustments?date_start={start_date}&date_end={end_date}&organization_id={org_id}"
        mov_url = f"{api_domain}/books/v3/stocktransfers?date_start={start_date}&date_end={end_date}&organization_id={org_id}"
        sources.extend([adj_url, mov_url])
        _ = get_json(adj_url, headers)
        _ = get_json(mov_url, headers)

        result = {
            "period": {"start": start_date, "end": end_date},
            "transfers": [],
            "totals": {"count": 0, "qty": 0.0},
        }
    ...
    alerts = _validate_ib_transfer(result)
