...
LOGIC_META = {
    "id": "L-122",
    "title": "Round-Off Abuse Detector",
    "tags": ["round-off", "abuse", "controls"],
}


def _validate_round_off(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    total = float(result.get("totals", {}).get("amount", 0.0) or 0.0)
    sum_amt = 0.0
    for c in result.get("cases", []) or []:
        try:
            share = float(c.get("share_pct", 0.0) or 0.0)
            amt = float(c.get("amount", 0.0) or 0.0)
            if share < 0.0 or share > 100.0:
                alerts.append("share_out_of_bounds")
            sum_amt += amt
        except Exception:
            alerts.append("invalid_case")
    if abs(sum_amt - total) > 0.01:
        alerts.append("totals_mismatch")
    if len(result.get("cases", []) or []) > 5:
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    alerts = _validate_round_off(out)
