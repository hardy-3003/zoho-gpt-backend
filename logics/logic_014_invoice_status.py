...
ID: L-014
Tags: []
Required Inputs: schema://invoice_status.input.v1
Outputs: schema://invoice_status.output.v1
Assumptions: 
...
LOGIC_META = {
    "id": "L-014",
    "title": "Invoice Status",
    "tags": ["invoice", "status", "sales", "ar"],
}


def _validate_invoice_status(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    try:
        ...
        for k in ["draft", "sent", "partially_paid", "paid", "overdue"]:
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    alerts = _validate_invoice_status(result)
    ...
    try:
        ...
        pack = log_with_deltas_and_anomalies(
            "L-014", payload, out.get("result") or {}, prov
        )
        if pack.get("alerts"):
