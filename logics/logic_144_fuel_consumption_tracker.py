...
LOGIC_META = {
    "id": "L-144",
    "title": "Fuel Consumption Tracker",
    "tags": ["fuel", "consumption", "transport"],
}


def _validate_fuel(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    try:
        liters = float(result.get("liters", 0.0) or 0.0)
        cost = float(result.get("cost", 0.0) or 0.0)
        if liters < 0 or cost < 0:
            alerts.append("invalid_totals")
        eff = result.get("efficiency") or {}
        km = eff.get("km")
        kpl = eff.get("km_per_liter")
        if liters > 0 and km is not None:
            expected = float(km) / float(liters)
            if kpl is not None and abs(float(kpl) - expected) > 0.0001:
                alerts.append("kpl_mismatch")
    except Exception:
        alerts.append("invalid_efficiency")
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        fuel_url = f"{api_domain}/books/v3/bills?date_start={start_date}&date_end={end_date}&category=fuel&organization_id={org_id}"
        trips_url = f"{api_domain}/books/v3/fleet/trips?date_start={start_date}&date_end={end_date}&organization_id={org_id}"
        sources.extend([fuel_url, trips_url])

        _ = get_json(fuel_url, headers)
        ...
        res = {
            ...
            "liters": 0.0,
            "cost": 0.0,
            "efficiency": {"km": None, "km_per_liter": None},
        }
