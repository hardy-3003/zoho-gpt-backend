...
LOGIC_META = {
    "id": "L-071",
    "title": "Fund Transfer Logs",
    "tags": ["fund transfer", "cash", "bank"],
}


def _validate_transfers(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    ...
    for t in transfers:
        ...
        if t.get("from") == t.get("to") and t.get("from") is not None:
            alerts.append("same_from_to_account")
        sum_amt += amt
    if abs(sum_amt - float(totals.get("amount", 0.0) or 0.0)) > 0.01:
        alerts.append("amount_totals_mismatch")
    if int(totals.get("count", 0) or 0) != len(transfers):
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        bank_txn_url = f"{api_domain}/books/v3/banktransactions?type=transfer&date_start={start_date}&date_end={end_date}&organization_id={org_id}"
        sources.append(bank_txn_url)
        _ = get_json(bank_txn_url, headers)
    ...
    alerts = _validate_transfers(result)
