...
LOGIC_META = {
    "id": "L-157",
    "title": "Self-learning Prediction (Revenue/Expense)",
    "tags": ["forecast", "learn", "timeseries"],
}


def _validate_slp(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    try:
        for arr_key in ["revenue_forecast", "expense_forecast"]:
            for it in result.get(arr_key) or []:
                if float(it.get("value", 0.0) or 0.0) < 0:
                    alerts.append("negative_value")
        hz = int(result.get("horizon_months", 0) or 0)
        if hz < 0 or hz > 12:
            alerts.append("horizon_out_of_bounds")
    except Exception:
...
def _learn_from_history(
    ...
    try:
        signals.append(f"horizon:{int(result.get('horizon_months') or 0)}")
    except Exception:
        pass
    try:
        append_event(
            LOGIC_META["id"],
            {
                ...
                "signals": signals,
                "summary": {
                    "size": len(result.get("revenue_forecast") or [])
                    + len(result.get("expense_forecast") or [])
                },
            },
        )
    except Exception:
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        inv_url = f"{api_domain}/books/v3/invoices?date_start=&date_end={end_date}&organization_id={org_id}&aggregate=month"
        bill_url = f"{api_domain}/books/v3/bills?date_start=&date_end={end_date}&organization_id={org_id}&aggregate=month"
