...
ID: L-024
Tags: []
Required Inputs: schema://employee_cost_trends.input.v1
Outputs: schema://employee_cost_trends.output.v1
Assumptions: 
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        prov["figures"].update(
            make_provenance(
                totals={
                    "endpoint": "reports/employee_costs",
        ...
        pack = log_with_deltas_and_anomalies(
            "L-024", payload, out.get("result") or {}, prov
        )
        if pack.get("alerts"):
            out["alerts"] = list(out.get("alerts", [])) + pack.get("alerts", [])
        new_conf = _score(
            sample_size=max(1, len((out.get("result") or {}).get("months", []) or [])),
