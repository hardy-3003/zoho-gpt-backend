...
LOGIC_META = {
    "id": "L-066",
    "title": "Marketing Spend Efficiency",
    "tags": ["marketing", "efficiency", "roi"],
}


def _validate_mkt_eff(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    spend = float(result.get("spend", 0.0) or 0.0)
    attr = result.get("attributed_revenue")
    roi = result.get("roi_pct")
    if spend < 0:
        alerts.append("negative_spend")
    if attr is None:
        if roi is not None:
            alerts.append("roi_should_be_null_without_revenue")
    else:
        if float(attr) < 0:
            alerts.append("negative_attributed_revenue")
    return alerts
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    alerts = _validate_mkt_eff(result)
