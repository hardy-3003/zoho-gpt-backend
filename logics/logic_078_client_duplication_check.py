...
"""
Title: Client Duplication Check
ID: L-078
Tags: []
Required Inputs: schema://client_duplication_check.input.v1
Outputs: schema://client_duplication_check.output.v1
Assumptions: 
...
LOGIC_META = {
    "id": "L-078",
    "title": "Client Duplication Check",
    "tags": ["client", "duplicate", "master"],
}


def _validate_client_dupes(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    su = result.get("suspects", [])
    totals = result.get("totals", {})
    if int(totals.get("suspects", 0) or 0) != len(su):
        alerts.append("totals_mismatch")
    for s in su:
        if not s.get("reason"):
            alerts.append("missing_reason")
    return alerts
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        customers_url = f"{api_domain}/books/v3/customers?organization_id={org_id}"
        sources.append(customers_url)
    ...
    alerts = _validate_client_dupes(result)
