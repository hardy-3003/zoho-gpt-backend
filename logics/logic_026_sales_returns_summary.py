...
ID: L-026
Tags: []
Required Inputs: schema://sales_returns_summary.input.v1
Outputs: schema://sales_returns_summary.output.v1
Assumptions: 
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        prov["figures"].update(
            make_provenance(
                totals={
                    "endpoint": "reports/sales_returns",
        ...
        pack = log_with_deltas_and_anomalies(
            "L-026", payload, out.get("result") or {}, prov
        )
        if pack.get("alerts"):
            out["alerts"] = list(out.get("alerts", [])) + pack.get("alerts", [])
        new_conf = _score(
            sample_size=max(
                1, len((out.get("result") or {}).get("customers", []) or [])
            ),
