...
LOGIC_META = {
    "id": "L-088",
    "title": "Profit Leakage Detector",
    "tags": ["leakage", "profit", "alerts"],
}

_LEAK_TYPES = {"pricing_error", "unbilled", "underbilling", "waived_charges"}


def _validate_leaks(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    total = float(result.get("totals", {}).get("amount", 0.0) or 0.0)
    sum_amt = 0.0
    for l in result.get("leaks", []):
        t = l.get("type")
        a = float(l.get("amount", 0.0) or 0.0)
        if t not in _LEAK_TYPES:
