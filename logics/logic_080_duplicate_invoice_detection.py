...
LOGIC_META = {
    "id": "L-080",
    "title": "Duplicate Invoice Detection",
    "tags": ["duplicate", "invoice", "ar"],
}

_REASONS = {"same_number", "same_amount", "same_date"}


def _validate_duplicate_invoices(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    dups = result.get("duplicates", [])
    totals = result.get("totals", {})
    if int(totals.get("duplicates", 0) or 0) != len(dups):
        alerts.append("totals_mismatch")
    seen = set()
    for d in dups:
        if d.get("reason") not in _REASONS:
            alerts.append("invalid_reason")
        pair = (d.get("invoice"), d.get("pair_with"))
        if pair[0] == pair[1]:
            alerts.append("self_pair")
        if pair in seen:
            alerts.append("duplicate_pair")
        seen.add(pair)
    return alerts
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    alerts = _validate_duplicate_invoices(result)
