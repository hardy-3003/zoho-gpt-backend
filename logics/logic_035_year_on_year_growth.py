...
ID: L-035
Tags: []
Required Inputs: schema://year_on_year_growth.input.v1
Outputs: schema://year_on_year_growth.output.v1
Assumptions: 
...
LOGIC_META = {
    "id": "L-035",
    "title": "Year-on-Year Growth",
    "tags": ["yoy", "growth", "revenue", "finance"],
}


def _validate_yoy(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    years = result.get("years", [])
    prev = None
    for y in years:
        rev = float(y.get("revenue", 0.0) or 0.0)
        if rev < 0:
            alerts.append(f"negative_revenue:{y.get('year','')}")
        yoy = y.get("yoy_pct")
        if prev is None and yoy is not None:
            alerts.append("yoy_without_prev")
        prev = rev
    latest = result.get("totals", {}).get("latest")
    return alerts
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        result = {
            ...
            "years": [],
            "totals": {"latest": 0.0, "yoy_pct": None},
        }
