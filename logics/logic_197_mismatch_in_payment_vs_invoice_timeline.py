...
LOGIC_META = {
    "id": "L-197",
    "title": "Mismatch in Payment vs Invoice Timeline",
    "tags": ["timeline", "payments", "invoices"],
}


def _validate_timeline(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    for row in result.get("mismatches", []) or []:
        try:
            dd = float(row.get("days_diff", 0.0))
            if abs(dd) > 90:
                alerts.append("large_timeline_gap")
        except Exception:
            alerts.append("invalid_days_diff")
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    alerts = _validate_timeline(result)
