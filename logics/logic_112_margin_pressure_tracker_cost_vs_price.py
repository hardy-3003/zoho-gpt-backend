...
LOGIC_META = {
    "id": "L-112",
    "title": "Margin Pressure Tracker (Cost vs Price)",
    "tags": ["margin", "price", "cogs", "sales"],
}


def _validate_margin(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    for it in result.get("items", []) or []:
        try:
            margin_pct = float(it.get("margin_pct", 0.0) or 0.0)
            if margin_pct < -100.0 or margin_pct > 100.0:
                alerts.append("margin_out_of_bounds")
            if margin_pct < 10.0:
                alerts.append("low_margin")
        except Exception:
            alerts.append("margin_invalid")
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        result = {
            ...
            "items": [],
            "totals": {"weighted_margin_pct": 0.0},
        }
    ...
    alerts = _validate_margin(result)
