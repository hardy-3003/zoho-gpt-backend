...
"""
Title: Auto Reconciliation Suggestion
ID: L-103
Tags: []
Required Inputs: schema://auto_reconciliation_suggestion.input.v1
Outputs: schema://auto_reconciliation_suggestion.output.v1
Assumptions: 
...
LOGIC_META = {
    "id": "L-103",
    "title": "Auto Reconciliation Suggestion",
    "tags": ["recon", "matching", "bank"],
}


def _validate_auto_recon(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    for m in result.get("matches", []):
        conf = float(m.get("confidence_pct", 0.0) or 0.0)
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        gl_url = f"{api_domain}/books/v3/journals?date_start={start_date}&date_end={end_date}&organization_id={org_id}"
        sources.extend([bank_url, gl_url])
        _ = get_json(bank_url, headers)
        _ = get_json(gl_url, headers)
    ...
    alerts = _validate_auto_recon(result)
