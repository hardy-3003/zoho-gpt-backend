...
ID: L-023
Tags: []
Required Inputs: schema://item_wise_profitability.input.v1
Outputs: schema://item_wise_profitability.output.v1
Assumptions: 
...
LOGIC_META = {
    "id": "L-023",
    "title": "Item-wise Profitability",
    "tags": ["items", "profitability", "margin", "cogs"],
}


def _validate_item_profitability(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    try:
        for it in result.get("items", []) or []:
            ...
            if abs(margin - (revenue - cost)) > 0.01:
                ...
                break
            if margin < -1e-6:
                alerts.append("negative margin detected")
                break
    except Exception:
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        sources.append("/books/v3/bills?date_start=&date_end=&item_id=")
        result = {
    ...
    alerts = _validate_item_profitability(result)
    ...
    try:
        ...
        pack = log_with_deltas_and_anomalies(
            "L-023", payload, out.get("result") or {}, prov
        )
        if pack.get("alerts"):
