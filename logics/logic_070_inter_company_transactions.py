...
LOGIC_META = {
    "id": "L-070",
    "title": "Inter-company Transactions",
    "tags": ["intercompany", "ic", "transactions"],
}


def _validate_ic_txn(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    ...
    for e in entries:
        ...
        net = float(e.get("net", debit - credit))
        if abs(net - (debit - credit)) > 0.01:
            alerts.append(f"net_mismatch:{e.get('counterparty','')}")
        sum_net += net
    if abs(sum_net - float(totals.get("net", 0.0) or 0.0)) > 0.01:
        alerts.append("totals_mismatch")
    for e in entries:
        if abs(float(e.get("net", 0.0) or 0.0)) > 1e6:
            alerts.append(f"large_unmatched_net:{e.get('counterparty','')}")
    return alerts
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    alerts = _validate_ic_txn(result)
