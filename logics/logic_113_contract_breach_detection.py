...
LOGIC_META = {
    "id": "L-113",
    "title": "Contract Breach Detection",
    "tags": ["contract", "breach", "sla"],
}


def _validate_breach(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    allowed = {"qty", "date", "price", "quality"}
    seen_contracts: Dict[str, int] = {}
    for b in result.get("breaches", []) or []:
        if b.get("type") not in allowed:
            alerts.append("invalid_type")
        c = str(b.get("contract", ""))
        if c:
            seen_contracts[c] = seen_contracts.get(c, 0) + 1
    if any(v > 1 for v in seen_contracts.values()):
        alerts.append("repeat_breach")
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    alerts = _validate_breach(result)
