...
LOGIC_META = {
    "id": "L-183",
    "title": "Related Party Transaction Tracker",
    "tags": ["party", "related", "audit"],
}


def _validate_rpt(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    whitelist = {"sale", "purchase", "loan", "service"}
    for row in result.get("transactions", []) or []:
        t = row.get("type")
        if t and t not in whitelist:
            alerts.append("invalid_type")
    totals = result.get("totals") or {}
    try:
        if float(totals.get("amount", 0.0)) < 0:
            alerts.append("negative_total_amount")
    except Exception:
