...
LOGIC_META = {
    "id": "L-196",
    "title": "P&L Ratio Anomaly Finder",
    "tags": ["pnl", "ratio", "anomaly"],
}


def _validate_ratios(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    for r in result.get("ratios", []) or []:
        try:
            z = float(r.get("zscore", 0.0))
            if abs(z) > 3.0:
                alerts.append("ratio_outlier")
        except Exception:
