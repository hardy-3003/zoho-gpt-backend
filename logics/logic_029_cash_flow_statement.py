...
ID: L-029
Tags: []
Required Inputs: schema://cash_flow_statement.input.v1
Outputs: schema://cash_flow_statement.output.v1
Assumptions: 
...
def _validate_cfs(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    try:
        sections = result.get("sections", {}) or {}
        net = float(result.get("totals", {}).get("net_change", 0) or 0)
        total_sections = (
            float(sections.get("operating", 0) or 0)
            + float(sections.get("investing", 0) or 0)
            + float(sections.get("financing", 0) or 0)
        )
        if abs(total_sections - net) > 0.01:
            alerts.append("sections sum != net_change")
    except Exception:
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        sources.append("/books/v3/journals?date_start=&date_end=")
        sources.append("/books/v3/banktransactions?date_start=&date_end=")
        result = {
            "period": {"start_date": start_date, "end_date": end_date},
            "sections": {"operating": 0.0, "investing": 0.0, "financing": 0.0},
            "totals": {"net_change": 0.0},
        }
    ...
    try:
        ...
        pack = log_with_deltas_and_anomalies(
            "L-029", payload, out.get("result") or {}, prov
        )
        if pack.get("alerts"):
