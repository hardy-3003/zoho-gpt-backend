...
LOGIC_META = {
    "id": "L-079",
    "title": "Data Entry Consistency",
    "tags": ["consistency", "validation", "hygiene"],
}


def _validate_data_consistency(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    issues = result.get("issues", [])
    totals = result.get("totals", {})
    if int(totals.get("issues", 0) or 0) != len(issues):
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    alerts = _validate_data_consistency(result)
