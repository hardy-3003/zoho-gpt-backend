...
def _validate_bounce(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    for inv in result.get("invoices", []) or []:
        try:
            p = float(inv.get("prob_bounce", 0.0) or 0.0)
            if p < 0.0 or p > 1.0:
                alerts.append("prob_out_of_bounds")
            expected = _band(p)
            if inv.get("risk_band") != expected:
                alerts.append("band_mismatch")
        except Exception:
            alerts.append("prob_invalid")
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        pay_url = f"{api_domain}/books/v3/customerpayments?date_start={start_date}&date_end={end_date}&organization_id={org_id}"
        sources.extend([inv_url, pay_url])
        _ = get_json(inv_url, headers)
        _ = get_json(pay_url, headers)
    ...
    alerts = _validate_bounce(result)
