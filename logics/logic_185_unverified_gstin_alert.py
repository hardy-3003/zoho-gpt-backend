...
LOGIC_META = {
    "id": "L-185",
    "title": "Unverified GSTIN Alert",
    "tags": ["gst", "compliance", "validation"],
}


def _validate_gstin(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    for row in result.get("parties", []) or []:
        gstin = (row.get("gstin") or "").strip()
        if not gstin or len(gstin) < 10:
            alerts.append("invalid_gstin")
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        contacts_url = f"{api_domain}/books/v3/contacts?organization_id={org_id}"
        sources.append(contacts_url)
        _ = get_json(contacts_url, headers)
