...
def _validate_far(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    assets = result.get("assets", [])
    total_nbv = float(result.get("totals", {}).get("nbv", 0.0) or 0.0)
    sum_nbv = 0.0
    for a in assets:
        cost = float(a.get("cost", 0.0) or 0.0)
...
def handle(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        journals_url = f"{api_domain}/books/v3/journals?date_end={end_date}&organization_id={org_id}"
        sources.extend([fa_url, journals_url])
