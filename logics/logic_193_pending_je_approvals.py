...
"""
Title: Pending Je Approvals
ID: L-193
Tags: []
Required Inputs: schema://pending_je_approvals.input.v1
Outputs: schema://pending_je_approvals.output.v1
Assumptions: 
...
LOGIC_META = {
    "id": "L-193",
    "title": "Pending JE Approvals",
    "tags": ["journal", "approvals", "pending"],
}
...
def _validate_pending(result: Dict[str, Any]) -> List[str]:
    ...
    for row in result.get("pending", []) or []:
        try:
            age = float(row.get("age_days", 0.0))
            if age < 0:
                alerts.append("negative_age")
        except Exception:
            alerts.append("invalid_age")
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        je_url = f"{api_domain}/books/v3/journals?status=pending&date_start={start_date}&date_end={end_date}&organization_id={org_id}"
