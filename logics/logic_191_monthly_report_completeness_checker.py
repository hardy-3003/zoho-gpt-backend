...
LOGIC_META = {
    "id": "L-191",
    "title": "Monthly Report Completeness Checker",
    "tags": ["completeness", "reports"],
}


def _validate_completeness(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    try:
        req = int((result.get("basis") or {}).get("required", 0))
        prod = int((result.get("basis") or {}).get("produced", 0))
        if req < 0 or prod < 0:
            alerts.append("invalid_basis")
        if prod < req:
            alerts.append("missing_reports")
    except Exception:
        alerts.append("invalid_numbers")
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    alerts = _validate_completeness(result)
