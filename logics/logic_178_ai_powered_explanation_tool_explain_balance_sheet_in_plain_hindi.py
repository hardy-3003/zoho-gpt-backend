...
LOGIC_META = {
    "id": "L-178",
    "title": "AI-Powered Explanation Tool (Plain Hindi)",
    "tags": ["explain", "hindi", "education"],
}


def _validate_explain_hi(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    topics = {"balance_sheet", "assets", "liabilities", "equity", "cash_flow"}
    expl = result.get("explanations") or []
    for e in expl:
        if e.get("topic") not in topics:
            alerts.append("invalid_topic")
        if not (e.get("text_hi") or "").strip():
            alerts.append("empty_text")
    if result.get("style") != "simple-hindi":
        alerts.append("invalid_style")
...
def _learn_from_history(
    ...
    signals: List[str] = ["l4-v0-run", "schema:stable", "lang:hi"]
    try:
        signals.append(f"topics:{len(result.get('explanations') or [])}")
        if alerts:
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        explanations: List[Dict[str, Any]] = [
            {
                "topic": "balance_sheet",
                "text_hi": "Balance Sheet aapke vyavsay ki sampatti aur zimmedariyon ka snapshot hota hai.",
            },
            {
                "topic": "assets",
                "text_hi": "Assets woh cheezen hain jo company ke paas hain aur bhavishy me fayda deti hain.",
            },
            {
                "topic": "liabilities",
                "text_hi": "Liabilities udhaar ya zimmedariyaan hoti hain jinko aage jaakar chukana hota hai.",
            },
            {
                "topic": "equity",
                "text_hi": "Equity malik ki hissaidari hoti hai, yaani assets minus liabilities.",
            },
            {
                "topic": "cash_flow",
                "text_hi": "Cash Flow report dikhati hai paise ka aana jaana, jo daily operations ke liye zaroori hai.",
            },
        ]
        result = {
    ...
    alerts = _validate_explain_hi(result)
