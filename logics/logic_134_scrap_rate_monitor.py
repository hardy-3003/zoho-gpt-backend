...
LOGIC_META = {
    "id": "L-134",
    "title": "Scrap Rate Monitor",
    "tags": ["scrap", "waste", "production"],
}


def _validate_scrap(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    try:
        sp = result.get("scrap_pct")
        if sp is not None:
            s = float(sp)
            if s < 0 or s > 100:
                alerts.append("scrap_out_of_bounds")
        for b in result.get("batches", []) or []:
            qp = b.get("scrap_pct")
            if qp is not None:
                p = float(qp)
                if p < 0 or p > 100:
                    alerts.append("batch_scrap_out_of_bounds")
                if p > 10:
                    alerts.append("high_batch_scrap")
    except Exception:
