...
ID: L-109
Tags: []
Required Inputs: schema://pending_approvals_summary.input.v1
Outputs: schema://pending_approvals_summary.output.v1
Assumptions: 
...
def _validate_pending(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    allowed = {"invoice", "bill", "je", "po"}
    for p in result.get("pending", []) or []:
        if p.get("type") not in allowed:
            alerts.append("invalid_type")
        try:
            if int(p.get("age_days", 0)) > 7:
                alerts.append("aged_gt_7_days")
        except Exception:
            alerts.append("age_invalid")
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        # representative approvals endpoints
        inv_url = (
            f"{api_domain}/books/v3/invoices?status=draft&organization_id={org_id}"
        )
        bill_url = f"{api_domain}/books/v3/bills?status=draft&organization_id={org_id}"
        je_url = (
            f"{api_domain}/books/v3/journals?status=pending&organization_id={org_id}"
        )
        po_url = f"{api_domain}/books/v3/purchaseorders?status=awaiting_approval&organization_id={org_id}"
        sources.extend([inv_url, bill_url, je_url, po_url])
    ...
    alerts = _validate_pending(result)
