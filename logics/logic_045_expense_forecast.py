...
LOGIC_META = {
    "id": "L-045",
    "title": "Expense Forecast",
    "tags": ["forecast", "expenses"],
}


def _validate_expense_forecast(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    forecast = result.get("forecast", [])
    if any((f.get("expense", 0.0) or 0.0) < 0 for f in forecast):
        alerts.append("negative_expense")
    if len(forecast) < 3:
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    alerts = _validate_expense_forecast(result)
