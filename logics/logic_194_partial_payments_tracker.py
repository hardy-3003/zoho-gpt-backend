...
LOGIC_META = {
    "id": "L-194",
    "title": "Partial Payments Tracker",
    "tags": ["payments", "receivables", "partial"],
}


def _validate_partial(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    for row in result.get("entries", []) or []:
        try:
            ...
            if abs(balance - (amount - paid)) > 0.0001:
                alerts.append("balance_mismatch")
        except Exception:
            alerts.append("invalid_amounts")
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    alerts = _validate_partial(result)
