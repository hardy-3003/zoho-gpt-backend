...
def _validate_lead_ratio(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    leads = int(result.get("leads", 0) or 0)
    clients = int(result.get("clients", 0) or 0)
    ratio = result.get("ratio")
    if leads > 0:
        expected = clients / leads
        if ratio is not None and abs(float(ratio) - expected) > 1e-6:
            alerts.append("ratio_mismatch")
    else:
        if ratio is not None:
            alerts.append("ratio_should_be_null_when_no_leads")
    if clients < 0 or leads < 0:
        alerts.append("negative_counts")
    if ratio is not None and ratio < 0.0:
        alerts.append("negative_ratio")
    if ratio is not None and ratio < 0.1 and leads > 0:
        alerts.append("low_ratio")
    return alerts
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        sources.extend([est_url, customers_url])
    ...
    alerts = _validate_lead_ratio(result)
