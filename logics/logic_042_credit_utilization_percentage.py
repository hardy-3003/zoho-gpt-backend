...
ID: L-042
Tags: []
Required Inputs: schema://credit_utilization_percentage.input.v1
Outputs: schema://credit_utilization_percentage.output.v1
Assumptions:
...
LOGIC_META = {
    ...
    "tags": ["credit", "utilization", "ap", "ar"],
}


def _validate_credit_util(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    util = result.get("utilization", {})
    for k in ["ap_pct", "ar_pct"]:
        v = util.get(k)
        if v is None:
            continue
        if v < 0 or v > 200:
            alerts.append(f"{k}_out_of_bounds")
    return alerts
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        result = {
            ...
            "utilization": {"ap_pct": None, "ar_pct": None},
            "notes": [],
        }
    ...
    alerts = _validate_credit_util(result)
