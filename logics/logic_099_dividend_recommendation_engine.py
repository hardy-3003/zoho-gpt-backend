...
LOGIC_META = {
    "id": "L-099",
    "title": "Dividend Recommendation",
    "tags": ["dividend", "payout", "policy"],
}


def _validate_dividend(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    rec = result.get("recommendation", {})
    pct = float(rec.get("payout_pct", 0.0) or 0.0)
    amt = float(rec.get("amount", 0.0) or 0.0)
    if pct < 0 or pct > 100:
        alerts.append("payout_pct_out_of_bounds")
    if amt < 0:
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        result = {
            ...
            "recommendation": {"payout_pct": 0.0, "amount": 0.0},
            "constraints": [],
        }
    ...
    alerts = _validate_dividend(result)
