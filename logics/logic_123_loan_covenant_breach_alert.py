...
"""
Title: Loan Covenant Breach Alert
ID: L-123
Tags: []
Required Inputs: schema://loan_covenant_breach_alert.input.v1
Outputs: schema://loan_covenant_breach_alert.output.v1
Assumptions: 
...
LOGIC_META = {
    "id": "L-123",
    "title": "Loan Covenant Breach Alert",
    "tags": ["covenant", "bank", "ratio"],
}


def _validate_covenants(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    for c in result.get("covenants", []) or []:
        try:
            thr = float(c.get("threshold", 0.0) or 0.0)
            act = float(c.get("actual", 0.0) or 0.0)
            computed = "ok" if act >= thr else "breach"
            if c.get("status") != computed:
                alerts.append("status_mismatch")
        except Exception:
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        bs_url = f"{api_domain}/books/v3/chartofaccounts?organization_id={org_id}"
        fin_url = f"{api_domain}/books/v3/reports/profitandloss?date_start={start_date}&date_end={end_date}&organization_id={org_id}"
        sources.extend([bs_url, fin_url])
        _ = get_json(bs_url, headers)
        _ = get_json(fin_url, headers)
    ...
    alerts = _validate_covenants(out)
