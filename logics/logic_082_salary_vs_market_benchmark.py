...
LOGIC_META = {
    "id": "L-082",
    "title": "Salary vs Market Benchmark",
    "tags": ["salary", "benchmark", "hr"],
}


def _validate_salary_benchmark(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    for r in result.get("roles", []):
        ai = float(r.get("avg_internal", 0.0) or 0.0)
        mr = r.get("market_ref")
        dp = r.get("delta_pct")
        if ai < 0:
            alerts.append("negative_internal_avg")
        if mr is not None and float(mr) < 0:
            alerts.append("negative_market_ref")
        if dp is not None and (float(dp) < -1000 or float(dp) > 1000):
            alerts.append("delta_pct_out_of_bounds")
    return alerts
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        # No direct salary endpoint in Books; touch settings for metadata presence
        ...
        result = {
            ...
            "roles": [],
            "notes": ["market data unavailable; roles left empty"],
        }
    ...
    alerts = _validate_salary_benchmark(result)
