...
LOGIC_META = {
    ...
    "tags": ["mom", "comparison", "finance"],
}


def _validate_mom(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    months = result.get("months", [])
    for m in months:
        rev = float(m.get("revenue", 0.0) or 0.0)
        exp = float(m.get("expense", 0.0) or 0.0)
        prof = float(m.get("profit", 0.0) or 0.0)
        if abs(prof - (rev - exp)) > 0.01:
            alerts.append(f"profit_mismatch:{m.get('month','')}")
    return alerts
