...
LOGIC_META = {
    "id": "L-135",
    "title": "Machine Efficiency Tracker (OEE)",
    "tags": ["machine", "efficiency", "oee"],
}


def _validate_oee(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    try:
        avg = result.get("avg_oee_pct")
        if avg is not None:
            a = float(avg)
            if a < 0 or a > 100:
                alerts.append("avg_oee_out_of_bounds")
        for m in result.get("machines", []) or []:
            o = float(m.get("oee_pct", 0.0))
            if o < 0 or o > 100:
                alerts.append("oee_out_of_bounds")
            if o < 60:
                alerts.append("oee_below_threshold")
    except Exception:
        alerts.append("invalid_machines")
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        machines_url = f"{api_domain}/books/v3/production/machines?date_start={start_date}&date_end={end_date}&organization_id={org_id}"
        sources.append(machines_url)

        _ = get_json(machines_url, headers)
