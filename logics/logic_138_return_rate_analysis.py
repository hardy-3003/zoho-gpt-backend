...
LOGIC_META = {
    "id": "L-138",
    "title": "Return Rate Analysis",
    "tags": ["returns", "customers", "rate"],
}


def _validate_returns(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    try:
        rp = result.get("return_pct")
        if rp is not None:
            r = float(rp)
            if r < 0 or r > 100:
                alerts.append("return_pct_out_of_bounds")
        for c in result.get("customers", []) or []:
            cp = c.get("return_pct")
            if cp is not None:
                p = float(cp)
                if p < 0 or p > 100:
                    alerts.append("customer_return_pct_out_of_bounds")
                if p > 15:
                    alerts.append("high_customer_return_rate")
    except Exception:
        alerts.append("invalid_customers")
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        cn_url = f"{api_domain}/books/v3/creditnotes?date_start={start_date}&date_end={end_date}&organization_id={org_id}"
        inv_url = f"{api_domain}/books/v3/invoices?date_start={start_date}&date_end={end_date}&organization_id={org_id}"
        sources.extend([cn_url, inv_url])

        _ = get_json(cn_url, headers)
    ...
    alerts = _validate_returns(result)
