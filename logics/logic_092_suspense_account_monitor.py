...
LOGIC_META = {
    "id": "L-092",
    "title": "Suspense Account Monitor",
    "tags": ["suspense", "clearing", "recon"],
}


def _validate_suspense(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    ...
    for e in result.get("entries", []):
        ...
        sum_amt += amt
        if int(e.get("age_days", 0) or 0) > 30:
            aged_over_30 = True
    if abs(sum_amt - total) > 0.01:
        alerts.append("totals_mismatch")
    if aged_over_30:
        alerts.append("aged_entries_over_30")
    return alerts
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    alerts = _validate_suspense(result)
