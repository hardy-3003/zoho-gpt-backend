...
ID: L-040
Tags: []
Required Inputs: schema://gst_reconciliation_status.input.v1
Outputs: schema://gst_reconciliation_status.output.v1
Assumptions: 
...
LOGIC_META = {
    "id": "L-040",
    "title": "GST Reconciliation Status",
    "tags": ["gst", "reconciliation", "compliance"],
}


def _validate_gst_recon(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    status = result.get("status", {})
    matched = float(status.get("matched", 0.0) or 0.0)
    mismatched = float(status.get("mismatched", 0.0) or 0.0)
    pending = float(status.get("pending", 0.0) or 0.0)
    total = matched + mismatched + pending
    declared = total  # placeholder reconciliation reference
    if abs(total - declared) > 0.01:
        alerts.append("declared_totals_mismatch")
    return alerts
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        returns_url = f"{api_domain}/books/v3/taxes/gst/returns?organization_id={org_id}&date_start={start_date}&date_end={end_date}"
        ...
        sources.extend([returns_url, inv_url, bill_url])
        _ = get_json(returns_url, headers)
        _ = get_json(inv_url, headers)
        _ = get_json(bill_url, headers)

        result = {
            "period": {"start": start_date, "end": end_date},
            "status": {"matched": 0.0, "mismatched": 0.0, "pending": 0.0},
            "notes": [],
        }
    ...
    alerts = _validate_gst_recon(result)
