...
"""
Title: Capital Invested Timeline
ID: L-052
Tags: []
Required Inputs: schema://capital_invested_timeline.input.v1
Outputs: schema://capital_invested_timeline.output.v1
Assumptions:
...
LOGIC_META = {
    "id": "L-052",
    "title": "Capital Invested Timeline",
    "tags": ["capital", "equity", "timeline"],
}


def _validate_capital_timeline(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    events = result.get("events", [])
    closing = float(result.get("closing", 0.0) or 0.0)
    delta = 0.0
    for ev in events:
        amt = float(ev.get("amount", 0.0) or 0.0)
        typ = ev.get("type")
        if amt < 0:
        ...
        if typ == "injection":
            ...
            delta -= amt
    # Cannot validate against opening (unknown), but closing should be >= 0
    if closing < 0:
        alerts.append("negative_closing")
    return alerts
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    alerts = _validate_capital_timeline(result)
