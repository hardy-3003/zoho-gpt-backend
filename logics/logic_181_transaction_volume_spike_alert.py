...
LOGIC_META = {
    "id": "L-181",
    "title": "Transaction Volume Spike Alert",
    "tags": ["audit", "spike", "volume"],
}


def _validate_spike(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    for row in result.get("spikes", []) or []:
        try:
            z = float(row.get("zscore", 0.0))
            if abs(z) > 3.0:
                alerts.append("spike_detected")
        except Exception:
            alerts.append("invalid_zscore")
    totals = result.get("totals") or {}
    try:
        if int(totals.get("count", 0)) < 0:
            alerts.append("negative_count")
    except Exception:
