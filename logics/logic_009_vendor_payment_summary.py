...
ID: L-009
Tags: []
Required Inputs: schema://vendor_payment_summary.input.v1
Outputs: schema://vendor_payment_summary.output.v1
Assumptions: 
...
LOGIC_META = {
    "id": "L-009",
    "title": "Vendor Payment Summary",
    "tags": ["vendors", "payments", "outflow"],
}


def _validate_vendor_payments(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    try:
        ...
        if total_amount > 0:
            for v in vendors:
                if float(v.get("amount", 0) or 0) / total_amount > 0.6:
                    alerts.append("vendor concentration >60%")
                    break
    except Exception:
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    alerts = _validate_vendor_payments(result)
    learn = _learn_from_history(payload, result)
    conf = 0.6
    if any(a in ("totals mismatch", "vendor concentration >60%") for a in alerts):
        conf -= 0.15
    ...
    try:
        ...
        pack = log_with_deltas_and_anomalies(
            "L-009", payload, out.get("result") or {}, prov
        )
        if pack.get("alerts"):
