...
LOGIC_META = {
    "id": "L-137",
    "title": "Quality Defect Rate",
    "tags": ["quality", "defect", "production"],
}


def _validate_defects(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    try:
        dp = result.get("defect_pct")
        if dp is not None:
            d = float(dp)
            if d < 0 or d > 100:
                alerts.append("defect_out_of_bounds")
        for ln in result.get("lines", []) or []:
            inspected = int(ln.get("inspected", 0))
            defects = int(ln.get("defects", 0))
            pct = ln.get("defect_pct")
            if inspected < 0 or defects < 0:
                alerts.append("negative_counts")
            if pct is not None:
                p = float(pct)
                if p < 0 or p > 100:
                    alerts.append("line_defect_out_of_bounds")
                if p > 5:
                    alerts.append("line_defect_high")
    except Exception:
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        insp_url = f"{api_domain}/books/v3/quality/inspections?date_start={start_date}&date_end={end_date}&organization_id={org_id}"
        sources.append(insp_url)

        _ = get_json(insp_url, headers)
    ...
    alerts = _validate_defects(result)
