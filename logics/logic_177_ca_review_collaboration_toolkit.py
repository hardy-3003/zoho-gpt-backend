...
LOGIC_META = {
    "id": "L-177",
    "title": "CA Review Collaboration Toolkit",
    "tags": ["collab", "review", "workflow"],
}


def _validate_cact(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    ...
    for t in threads:
        ...
        open_n += 1 if st == "open" else 0
        resolved_n += 1 if st == "resolved" else 0
    totals = result.get("totals") or {}
    if (
        int(totals.get("open", 0)) != open_n
        or int(totals.get("resolved", 0)) != resolved_n
    ):
...
def _learn_from_history(
    ...
    try:
        open_n = int((result.get("totals") or {}).get("open", 0))
        signals.append(f"open:{open_n}")
        if alerts:
