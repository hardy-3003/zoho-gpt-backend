...
LOGIC_META = {
    "id": "L-111",
    "title": "Late Vendor Payment Penalty Alert",
    "tags": ["penalty", "ap", "late"],
}


def _validate_vendor_penalty(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    for a in result.get("alerts", []) or []:
        try:
            days = int(a.get("days_late", 0))
            pen = float(a.get("penalty_est", 0.0) or 0.0)
            if days < 0 or pen < 0:
                alerts.append("negative_values")
            if days > 0:
                alerts.append("late_payment")
        except Exception:
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        payments_url = f"{api_domain}/books/v3/vendorpayments?date_start={start_date}&date_end={end_date}&organization_id={org_id}"
        sources.extend([bills_url, payments_url])
    ...
    alerts = _validate_vendor_penalty(result)
