...
"""
Title: Backdated Entry Detector
ID: L-110
Tags: []
Required Inputs: schema://backdated_entry_detector.input.v1
Outputs: schema://backdated_entry_detector.output.v1
Assumptions: 
...
LOGIC_META = {
    "id": "L-110",
    "title": "Backdated Entry Detector",
    "tags": ["backdated", "journals", "risk"],
}


def _validate_backdated(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    for e in result.get("entries", []) or []:
        try:
            days = int(e.get("backdated_days", 0))
            if days > 0:
                alerts.append("backdated_entry")
            if days > 30:
                alerts.append("critical_backdated_gt_30")
        except Exception:
            alerts.append("backdated_days_invalid")
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        j_url = f"{api_domain}/books/v3/journals?date_start={start_date}&date_end={end_date}&organization_id={org_id}"
        sources.append(j_url)
        _ = get_json(j_url, headers)
    ...
    alerts = _validate_backdated(result)
