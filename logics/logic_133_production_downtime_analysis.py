...
LOGIC_META = {
    "id": "L-133",
    "title": "Production Downtime Analysis",
    "tags": ["production", "downtime", "loss"],
}


def _validate_downtime(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    try:
        total = float(result.get("downtime_hours", 0.0) or 0.0)
        causes = result.get("causes", []) or []
        sum_hours = 0.0
        for c in causes:
            h = float(c.get("hours", 0.0) or 0.0)
            if h < 0:
                alerts.append("negative_hours")
            sum_hours += h
        if abs(sum_hours - total) > 0.0001:
            alerts.append("sum_mismatch")
        if total > 0:
            for c in causes:
                share = float(c.get("hours", 0.0) or 0.0) / total * 100.0
                if share > 60.0:
                    alerts.append("single_cause_dominant")
    except Exception:
        alerts.append("invalid_downtime")
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        incidents_url = f"{api_domain}/books/v3/production/incidents?date_start={start_date}&date_end={end_date}&organization_id={org_id}"
        sources.append(incidents_url)

        _ = get_json(incidents_url, headers)
    ...
    alerts = _validate_downtime(result)
