...
ID: L-050
Tags: []
Required Inputs: schema://loan_outstanding_summary.input.v1
Outputs: schema://loan_outstanding_summary.output.v1
Assumptions:
...
LOGIC_META = {
    "id": "L-050",
    "title": "Loan Outstanding Summary",
    "tags": ["loan", "principal", "interest", "liability"],
}


def _validate_loan_outstanding(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    loans = result.get("loans", [])
    totals_close = float(result.get("totals", {}).get("principal_close", 0.0) or 0.0)
    sum_close = 0.0
    for ln in loans:
        open_p = float(ln.get("principal_open", 0.0) or 0.0)
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        bank_url = f"{api_domain}/books/v3/bankaccounts?organization_id={org_id}"
        sources.extend([journals_url, bank_url])
    ...
    alerts = _validate_loan_outstanding(result)
