...
LOGIC_META = {
    "id": "L-121",
    "title": "Manual Journal Suspicion Detector",
    "tags": ["je", "suspicious", "controls"],
}


def _validate_mj(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    for s in result.get("suspects", []) or []:
        try:
            ...
            if sc > 0.5 and not (s.get("signals") or []):
                alerts.append("signals_missing_for_high")
        except Exception:
