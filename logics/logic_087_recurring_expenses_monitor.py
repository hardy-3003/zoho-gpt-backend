...
LOGIC_META = {
    "id": "L-087",
    "title": "Recurring Expenses Monitor",
    "tags": ["recurring", "expenses", "subscriptions"],
}

_FREQ = {"monthly", "annual"}


def _validate_recurring(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    total = float(result.get("totals", {}).get("amount_monthly_equiv", 0.0) or 0.0)
    sum_equiv = 0.0
    for s in result.get("subscriptions", []):
        freq = s.get("frequency")
        amt = float(s.get("amount", 0.0) or 0.0)
        if freq not in _FREQ:
            alerts.append("invalid_frequency")
        if amt < 0:
            alerts.append("negative_amount")
        monthly_equiv = (
            amt if freq == "monthly" else (amt / 12.0 if freq == "annual" else 0.0)
        )
        sum_equiv += monthly_equiv
    if abs(sum_equiv - total) > 0.01:
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        # No guaranteed recurring endpoint; touch settings as safe
        ...
        result = {
            ...
            "subscriptions": [],
            "totals": {"amount_monthly_equiv": 0.0},
        }
    ...
    alerts = _validate_recurring(result)
