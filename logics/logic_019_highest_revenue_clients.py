"""
Title: Highest Revenue Clients
ID: L-019
Tags: []
Required Inputs: schema://highest_revenue_clients.input.v1
Outputs: schema://highest_revenue_clients.output.v1
Assumptions: 
...
LOGIC_META = {
    "id": "L-019",
    "title": "Highest Revenue Clients",
    "tags": ["clients", "customers", "revenue", "top"],
}


def _validate_top_clients(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    try:
        clients = result.get("clients", []) or []
        totals = sum(float(c.get("amount", 0) or 0) for c in clients)
        if totals > 0:
            top_amount = float(result.get("top_client", {}).get("amount", 0) or 0)
            if top_amount / totals > 0.6:
                alerts.append("top client share >60%")
        # totals equal sum check
        declared_total = totals  # amounts section not explicitly present
        if abs(declared_total - totals) > 0.01:
...
def handle(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        result = {
            ...
            "clients": [],
            "top_client": {"customer": "", "amount": 0.0},
        }
    ...
    alerts = _validate_top_clients(result)
    ...
    try:
        ...
        prov["figures"].update(
            make_provenance(
                totals={"endpoint": "reports/clients", "filters": {"period": {"start": start_date, "end": end_date}}}
            )
        )
        out["provenance"] = prov

        pack = log_with_deltas_and_anomalies("L-019", payload, out.get("result") or {}, prov)
        if pack.get("alerts"):
            out["alerts"] = list(out.get("alerts", [])) + pack.get("alerts", [])
        new_conf = _score(
            sample_size=max(1, len((out.get("result") or {}).get("clients", []) or [])),
            anomalies=len(pack.get("anomalies", []) or []),
            validations_failed=1 if any("validation" in a for a in out.get("alerts", [])) else 0,
        )
