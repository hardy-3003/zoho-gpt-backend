...
ID: L-038
Tags: []
Required Inputs: schema://vendor_wise_spend.input.v1
Outputs: schema://vendor_wise_spend.output.v1
Assumptions: 
...
LOGIC_META = {
    "id": "L-038",
    "title": "Vendor-wise Spend",
    "tags": ["vendors", "spend", "ap"],
}


def _validate_vendor_spend(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    vendors = result.get("vendors", [])
    totals_amount = float(result.get("totals", {}).get("amount", 0.0) or 0.0)
    sum_amount = sum(float((v.get("amount", 0.0) or 0.0)) for v in vendors)
    if abs(totals_amount - sum_amount) > 0.01:
        alerts.append("totals_mismatch")
    if vendors:
        amounts = [float((v.get("amount", 0.0) or 0.0)) for v in vendors]
        total = sum(amounts)
        if total > 0:
            top = max(amounts)
            if top / total > 0.6:
                alerts.append("concentration_risk_top_vendor_gt_60pct")
    return alerts
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    alerts = _validate_vendor_spend(result)
