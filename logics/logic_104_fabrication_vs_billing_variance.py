...
"""
Title: Fabrication Vs Billing Variance
ID: L-104
Tags: []
Required Inputs: schema://fabrication_vs_billing_variance.input.v1
Outputs: schema://fabrication_vs_billing_variance.output.v1
Assumptions: 
...
LOGIC_META = {
    "id": "L-104",
    "title": "Fabrication vs Billing Variance",
    "tags": ["fabrication", "billing", "variance", "ops"],
}


def _validate_fabrication_billing(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    lines = result.get("lines", [])
    totals_var = float(result.get("totals", {}).get("variance_qty", 0.0) or 0.0)
    sum_var = 0.0
    for ln in lines:
        produced = float(ln.get("produced_qty", 0.0) or 0.0)
        billed = float(ln.get("billed_qty", 0.0) or 0.0)
        variance = float(ln.get("variance_qty", produced - billed))
        if abs(variance - (produced - billed)) > 0.01:
            alerts.append(f"variance_mismatch:{ln.get('item','')}")
        sum_var += variance
        if abs(variance) > max(10.0, 0.5 * (produced + billed)):
            alerts.append(f"large_variance:{ln.get('item','')}")
    if abs(sum_var - totals_var) > 0.01:
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    alerts = _validate_fabrication_billing(result)
