...
ID: L-003
Tags: []
Required Inputs: schema://trial_balance.input.v1
Outputs: schema://trial_balance.output.v1
Assumptions: 
...
LOGIC_META = {
    "id": "L-003",
    "title": "Trial Balance",
    "tags": ["trial balance", "debit", "credit", "accounts"],
}


def _validate_trial_balance(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    try:
        totals = result.get("totals", {}) or {}
        d = float(totals.get("debit", 0) or 0)
        c = float(totals.get("credit", 0) or 0)
        if abs(d - c) > 0.01:
            alerts.append("unbalanced: debits != credits")
    except Exception:
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    alerts = _validate_trial_balance(result)
