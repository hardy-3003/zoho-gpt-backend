...
LOGIC_META = {
    "id": "L-130",
    "title": "Order Fulfillment Cycle Time",
    "tags": ["fulfillment", "cycle-time", "orders"],
}


def _validate_cycle(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    try:
        for row in result.get("orders", []) or []:
            cd = row.get("cycle_days")
            if cd is not None and float(cd) < 0:
                alerts.append("negative_cycle_time")
        avg = result.get("avg_cycle_days")
        sla_days = 7.0
        if avg is not None and float(avg) > sla_days:
            alerts.append("avg_cycle_exceeds_sla")
    except Exception:
        alerts.append("invalid_orders")
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        sources.extend([so_url, inv_url, ship_url])
