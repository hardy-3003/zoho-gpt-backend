...
ID: L-010
Tags: []
Required Inputs: schema://receivables_summary.input.v1
Outputs: schema://receivables_summary.output.v1
Assumptions: 
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        pack = log_with_deltas_and_anomalies(
            "L-010", payload, out.get("result") or {}, prov
        )
        if pack.get("alerts"):
            out["alerts"] = list(out.get("alerts", [])) + pack.get("alerts", [])
        new_conf = _score(
            sample_size=max(
                1, len((out.get("result") or {}).get("top_debtors", []) or [])
            ),
