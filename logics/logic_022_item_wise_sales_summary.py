...
ID: L-022
Tags: []
Required Inputs: schema://item_wise_sales_summary.input.v1
Outputs: schema://item_wise_sales_summary.output.v1
Assumptions: 
...
LOGIC_META = {
    "id": "L-022",
    "title": "Item-wise Sales Summary",
    "tags": ["items", "sales", "summary"],
}


def _validate_item_sales(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    try:
        items = result.get("items", []) or []
        totals = result.get("totals", {}) or {}
        total_amount = float(totals.get("amount", 0) or 0)
        total_qty = float(totals.get("qty", 0) or 0)
        sum_amounts = 0.0
        sum_qty = 0.0
        for it in items:
            ...
            sum_amounts += amt
            sum_qty += qty
        if abs(sum_amounts - total_amount) > 0.01 or abs(sum_qty - total_qty) > 0.01:
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    alerts = _validate_item_sales(result)
    ...
    try:
        ...
        prov["figures"].update(
            make_provenance(
                totals={"endpoint": "reports/items", "filters": {"period": {"start": start_date, "end": end_date}}}
            )
        )
        out["provenance"] = prov

        pack = log_with_deltas_and_anomalies("L-022", payload, out.get("result") or {}, prov)
        if pack.get("alerts"):
