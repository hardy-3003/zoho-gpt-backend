...
"""
Title: Receivable Alerts
ID: L-048
Tags: []
Required Inputs: schema://receivable_alerts.input.v1
Outputs: schema://receivable_alerts.output.v1
Assumptions:
...
LOGIC_META = {
    "id": "L-048",
    "title": "Receivable Alerts",
    "tags": ["ar", "alerts", "aging"],
}

_ALERT_TYPES = {"overdue", "high_amount", "aging_bucket"}


def _validate_receivable_alerts(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        inv_url = f"{api_domain}/books/v3/invoices?status=unpaid&date_end={end_date}&organization_id={org_id}"
    ...
    alerts = _validate_receivable_alerts(result)
