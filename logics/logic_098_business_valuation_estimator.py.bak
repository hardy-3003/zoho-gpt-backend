...
LOGIC_META = {
    "id": "L-098",
    "title": "Business Valuation Estimator",
    "tags": ["valuation", "ev", "finance"],
}

_METHODS = {"SDE", "EBITDA", "RevenueMultiple"}


def _validate_valuation(result: Dict[str, Any]) -> List[str]:
    alerts: List[str] = []
    val = result.get("valuation", {})
    method = val.get("method")
    value = float(val.get("value", 0.0) or 0.0)
    if method not in _METHODS:
        alerts.append("invalid_method")
    if value < 0:
...
def handle(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        result = {
            ...
            "valuation": {"method": "RevenueMultiple", "value": 0.0},
            "assumptions": [],
        }
    ...
    alerts = _validate_valuation(result)
