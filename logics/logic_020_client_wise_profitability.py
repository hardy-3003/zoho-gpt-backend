...
ID: L-020
Tags: []
Required Inputs: schema://client_wise_profitability.input.v1
Outputs: schema://client_wise_profitability.output.v1
Assumptions: 
...
def handle_impl(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        prov["figures"].update(
            make_provenance(
                totals={"endpoint": "reports/client_profitability", "filters": {"period": {"start": start_date, "end": end_date}}}
            )
        )
        out["provenance"] = prov

        pack = log_with_deltas_and_anomalies("L-020", payload, out.get("result") or {}, prov)
        if pack.get("alerts"):
